...
from biopyc._image._channel._channel_set import IntensityChannelSet, LabelMaskChannelSet, BinaryMaskChannelSet
import dask.array as da 
from numpy import uint8
from pytest import raises

def test_biopyc_image_validation():
    intensity_channel_set = IntensityChannelSet([Channel(da.ones((5,5), dtype = uint8), "Stain1")])
    label_mask_channel_set = LabelMaskChannelSet([Channel(da.ones((5,5), dtype = uint8), "Stain1")])
    binary_mask_channel_set = BinaryMaskChannelSet([Channel(da.ones((5,5), dtype = bool), "Stain1")])
    ...
    with raises(TypeError, match="metadata should be a dictionary"):
        BiopycImage(intensity_channel_set, label_mask_channel_set, binary_mask_channel_set, 1)
...
def test_biopyc_image():
    ...
    bimg = BiopycImage(intensity_channel_set, label_mask_channel_set, binary_mask_channel_set, metadata)
