...
from biopyc._image._channel._channel_set import ChannelSet, IntensityChannelSet, LabelMaskChannelSet, BinaryMaskChannelSet
...
def test_channel_set_validation():
    ...
    with raises(ValueError, match="All channels should be of the same size"):
        ChannelSet([Channel(da.ones((5,5), dtype=uint8), "Stain1"), Channel(da.ones((6,6), dtype=uint8), "Stain2")])
    with raises(ValueError, match="All channels should have the same dtype"):
        ChannelSet([Channel(da.ones((5,5), dtype=uint8), "Stain1"), Channel(da.ones((5,5), dtype=int), "Stain2")])
    with raises(ValueError, match="Channels can't have duplicate names"):
        ChannelSet([Channel(da.ones((5,5), dtype=uint8), "Stain"), Channel(da.ones((5,5), dtype=uint8), "Stain")])
